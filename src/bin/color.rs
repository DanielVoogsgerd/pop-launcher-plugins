...
async fn main() {
    ...
    let mut plugin = UnitPlugin::new(database);
...
impl UnitPlugin {
    ...
    async fn search(&mut self, input: String) {
        let regex = match Regex::new("unit (?P<value>) (?P<unit_from>) to (?P<unit_to>)") {
            Ok(regex) => regex,
            Err(_) => {
                warn!("Could not compile regex");
                self.responder.respond(PluginResponse::Finished).await;
                return;
            }
        };

        let cap = match regex.captures(&input) {
            Some(cap) => cap,
            None => {
